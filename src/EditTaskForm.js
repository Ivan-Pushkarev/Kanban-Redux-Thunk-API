...
import {deleteCard, editCard, getEditCard} from './Redux/Actions';
import EditTaskFormModal from "./EditTaskFormModal";


function EditTask(props) {
   
    const initialCard = useSelector(state =>  state.editCardValue);
    const [card, setCard] = useState(initialCard);
    console.log( 'Edit card props' , props);
    
    useEffect(() => {
        props.getEditCard(props.match.params.cardId)
          }, []);
    ...
    const updateButtonHandler = () =>{
        props.editCard(props.match.params.cardId, card)
        history.push("/")
    }
...
export default withRouter(connect(null, {getEditCard, editCard, deleteCard})(EditTask));